...
def _generate_fallback_reading(dm: dict, pcts: dict, query: str | None) -> str:
    ...
    # 1. ความรัก / คู่ครอง / การแต่งงาน (Love & Spouse) - Check BEFORE career to avoid "แต่งงาน" -> "งาน"
    if any(k in query_str for k in ["ความรัก", "คู่ครอง", "แฟน", "แต่งงาน", "ความสัมพันธ์", "คนรัก", "คนคุย", "เนื้อคู่", "หย่า", "เลิก", "love", "marriage", "spouse"]):
        spouse_star = info["wealth_star"] if pol == "Yang" else info["career_star"]
        return (
            f"### 🔮 การวิเคราะห์ผังดวงจีนด้านความรักและคู่ครอง (BaZi Relationship Analysis)\n\n"
            f"- **ดิถีประจำตัว (Day Master)**: ดิถี {stem} ({elem}, {pol})\n"
            f"- **ดาวตัวแทนคู่ครอง**: {spouse_star}\n"
            f"- **เรือนคู่ครอง (日支)**: ฐานวันเกิดดวงชะตา\n"
            f"- **ประเด็นคำถามเจาะจง**: \"{query}\"\n\n"
            f"📌 **คำทำนายเจาะจงมิติความรักและคู่ครอง:**\n"
            f"สำหรับผังดวงชะตาดิถี {stem} ({elem}) ฐานเรือนคู่ครองส่งผลให้มีดวงชะตาคู่ครองที่เป็นคนมีเหตุผล มีความรับผิดชอบสูง และคอยเป็นที่ปรึกษาหนุนนำชีวิต\n\n"
            f"1. **ทิศทางความสัมพันธ์**: มีเกณฑ์ปรับความเข้าใจและพัฒนาความสัมพันธ์ไปสู่ความมั่นคงร่วมกัน\n"
    ...
    if any(k in query_str for k in ["อาชีพ", "การงาน", "ทำธุรกิจ", "ทำงาน", "ยศ", "ตำแหน่ง", "ย้ายงาน", "เปลี่ยนงาน", "สมัครงาน", "เปิดร้าน", "career", "job", "work", "business"]) or ("งาน" in query_str and "แต่งงาน" not in query_str):
        career_elem = "Fire" if elem == "Metal" else ("Metal" if elem == "Wood" else ("Earth" if elem == "Water" else ("Water" if elem == "Fire" else "Wood")))
        career_pct = pcts.get(career_elem, 20.0)
        return (
    ...
    # 3. การเงิน / โชคลาภ / ทรัพย์สิน (Wealth & Finance)
    if any(k in query_str for k in ["การเงิน", "เงิน", "โชคลาภ", "หุ้น", "ลงทุน", "รวย", "หนี้", "คริปโต", "กำไร", "wealth", "finance", "money"]):
        wealth_elem = "Wood" if elem == "Metal" else ("Earth" if elem == "Wood" else ("Fire" if elem == "Water" else ("Metal" if elem == "Fire" else "Water")))
        wealth_pct = pcts.get(wealth_elem, 20.0)
        return (
    ...
    # 4. สุขภาพ / ร่างกาย (Health & Vitality)
    if any(k in query_str for k in ["สุขภาพ", "ป่วย", "โรค", "ร่างกาย", "สายตา", "กระดูก", "ผ่าตัด", "อุบัติเหตุ", "health", "body"]):
        return (
            ...
            f"การปรับสมดุล 5 ธาตุสำหรับดิถี {stem} ({elem}) แนะนำให้ดูแล {info['health_focus']} และปรับการพักผ่อนให้เพียงพอเพื่อเสริมสร้างภูมิคุ้มกัน\n\n"
            f"1. **แนวทางดูแลสุขภาพ**: ควรรับประทานอาหารที่มีคุณสมบัติปรับสมดุล ออกกำลังกายอย่างสม่ำเสมอ และออกรับอากาศบริสุทธิ์"
        )

    # 5. ลูก / บุตรหลาน / บริวาร (Children & Offspring)
    if any(k in query_str for k in ["ลูก", "บุตร", "เด็ก", "บริวาร", "ครรภ์", "มีลูก", "child", "children", "son", "daughter"]):
        children_elem = "Water" if elem == "Metal" else ("Fire" if elem == "Wood" else ("Wood" if elem == "Water" else ("Earth" if elem == "Fire" else "Metal")))
        water_pct = pcts.get(children_elem, 20.0)
        return (
            ...
            f"1. **ลักษณะและวาสนาของบุตรหลาน**: บุตรหลานมีสติปัญญาเฉลียวฉลาด มีความคิดสร้างสรรค์สูง\n"
            f"2. **ความสัมพันธ์และการอุปถัมภ์**: เสายามในผังดวงชะตาส่งผลให้บุตรหลานมีความกตัญญูกตเวที\n"
            f"3. **ข้อแนะนำในการส่งเสริมพัฒนาการ**: ควรเน้นการสื่อสารด้วยความเข้าใจ เปิดโอกาสให้คิดและตัดสินใจด้วยตนเอง"
        )
    ...
    return (
        ...
        f"ดวงชะตานี้มีดิถีวันเป็น {stem} ({elem}) สำหรับคำถาม \"{query}\" มีพลังปรับสมดุลชีวิตร่วมกับธาตุ {lowest_elem1} และ {lowest_elem2} การดำเนินชีวิตการงาน การเงิน ความสัมพันธ์ และสุขภาพจะมีความราบรื่นและประสบความสำเร็จสูงเมื่อปรับยุทธศาสตร์ชีวิตตามสมดุล 5 ธาตุข้างต้น"
    )
