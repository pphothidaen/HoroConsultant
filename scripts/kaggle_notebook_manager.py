        ...
        "cells": [
            {
                ...
                "source": [
                    ...
                    "print('[MODEL] Removing incompatible torchao & installing fine-tuning packages...')\n",
                    "subprocess.run([sys.executable, '-m', 'pip', 'uninstall', '-y', 'torchao'], check=False)\n",
                    "subprocess.run([sys.executable, '-m', 'pip', 'install', '-q', '--progress-bar', 'off', '--prefer-binary', '-r', 'requirements.txt'], check=True)\n",
                    "subprocess.run([sys.executable, '-m', 'pip', 'install', '-q', '--progress-bar', 'off', '--prefer-binary', 'transformers==4.44.2', 'peft==0.12.0', 'bitsandbytes==0.43.3', 'datasets==2.18.0', 'trl==0.11.0', 'huggingface_hub==0.25.1', 'accelerate==0.33.0'], check=True)\n",
                    "import transformers, peft, trl, bitsandbytes, datasets, accelerate\n",
                    "print(f'[OK] Fail-Fast Import Verified: transformers={transformers.__version__}, peft={peft.__version__}, trl={trl.__version__}, accelerate={accelerate.__version__}')\n",
                    "\n",
