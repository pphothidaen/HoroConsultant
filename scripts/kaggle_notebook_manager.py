        ...
        "cells": [
            {
                ...
                "source": [
                    ...
                    "    if cap == (6, 0):\n",
                    "        print('[WARNING] Kaggle allocated Tesla P100 (sm_60). Installing PyTorch 2.1.2+cu121 compatibility wheel with native sm_60 Pascal support...')\n",
                    "        subprocess.run([sys.executable, '-m', 'pip', 'install', '-q', '--prefer-binary', 'torch==2.1.2', '--index-url', 'https://download.pytorch.org/whl/cu121'], check=True)\n",
                    "    else:\n",
                    ...
                    "subprocess.run([sys.executable, '-m', 'pip', 'install', '-q', '--progress-bar', 'off', '--prefer-binary', '-r', 'requirements.txt'], check=True)\n",
                    "subprocess.run([sys.executable, '-m', 'pip', 'install', '-q', '--progress-bar', 'off', '--prefer-binary', 'transformers==4.44.2', 'peft==0.12.0', 'trl==0.11.0', 'accelerate==0.33.0', 'bitsandbytes==0.43.3', 'datasets==2.18.0', 'huggingface_hub==0.25.1'], check=True)\n",
                    "try:\n",
                    "    import bitsandbytes\n",
