        ...
        "cells": [
            {
                ...
                "source": [
                    ...
                    "            print(f'\\u2139\\ufe0f Kaggle Secret note ({secret_key}): {e}')\n",
                    "except Exception as e:\n",
                    "    print(f'\\u2139\\ufe0f Kaggle Secrets Client not available: {e}')\n",
                    "\n",
                    ...
                    "    print(f'   Device Count: {torch.cuda.device_count()}, Device 0: {torch.cuda.get_device_name(0)} (sm_{cap[0]}{cap[1]})')\n",
                    "    if cap == (7, 5):\n",
                    "        print('\\u2139\\ufe0f Tesla T4 (sm_75) detected: float16 training, bitsandbytes 4-bit bypassed.')\n",
                    ...
                    "proc = subprocess.Popen([sys.executable, 'scripts/cloud_train_orchestrator.py', '--platform', '" + platform_arg + "', '--base-model', 'Qwen/Qwen2.5-7B-Instruct', '--epochs', '3'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, env=train_env)\n",
