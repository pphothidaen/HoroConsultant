        ...
        if event == "SessionStart":
            out = {
                "hookSpecificOutput": {
                    ...
                    "additionalContext": "Context handoff active. Primary authority resides in ATOMIC_TICKET.md (consolidated from atomic_tasks.md) and plans/plan.md.",
                }
            }
            ...
            out = {
                "systemMessage": "Context compaction completed. Resume from ATOMIC_TICKET.md (consolidated from atomic_tasks.md) and plans/plan.md."
            }
            sys.stdout.write(json.dumps(out) + "\n")
            return 0
...
def op_rehydrate(args: argparse.Namespace) -> int:
    ...
    cur_state = auth.get("current_state", "ATOMIC_TICKET.md")
