...
@pytest.mark.parametrize("failure_boundary", ("approval_consume", "provider"))
def test_capacity_released_when_execution_boundary_raises(tmp_path, monkeypatch, failure_boundary):
    """An admitted lease must not survive an exception before output parsing."""
    invocation = _capacity_bound_invocation(tmp_path)
    monkeypatch.setattr(command, "validate_execution_preflight", lambda _invocation: None)

    def fail_after_capacity_consumed(*args, **kwargs):
        state = json.loads((Path(invocation.capacity_store_path) / ".capacity.json").read_text())
        assert next(iter(state["leases"].values()))["requests_used"] == 1
        raise RuntimeError("injected execution boundary failure")

    if failure_boundary == "approval_consume":
        monkeypatch.setattr(command, "_consume_prepared_approval", fail_after_capacity_consumed)
        monkeypatch.setattr(command, "_run_provider_process", lambda *a, **k: pytest.fail("provider started after failed approval"))
    else:
        monkeypatch.setattr(command, "_run_provider_process", fail_after_capacity_consumed)

    with pytest.raises(RuntimeError, match="injected execution boundary failure"):
        command.execute_invocation(invocation)
    state = json.loads((Path(invocation.capacity_store_path) / ".capacity.json").read_text())
    assert state["leases"] == {}
    with pytest.raises(command.capacity.LeaseRejectedError, match="REPLAY_REJECTED"):
