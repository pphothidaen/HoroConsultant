...
def scan_sensitive(obj: Any) -> bool:
    ...
    if isinstance(obj, dict):
        for k, v in obj.items():
            if k in ("session", "prompt", "env", "credentials", "secrets", "api_key", "password"):
                return True
...
def op_hook(args: argparse.Namespace) -> int:
    ...
    if "tokens" in usage and isinstance(usage["tokens"], dict):
        ...
        try:
            if tpath.is_file() and not tpath.is_symlink() and not stat.S_ISFIFO(tpath.stat().st_mode):
    ...
    if signal["source"] in ("token_count", "percent"):
        ...
        if nbytes is None:
            level = "UNKNOWN"
        elif nbytes >= 900 * 1024:
            level = "CRITICAL"
        elif nbytes >= 450 * 1024:
            level = "SNAPSHOT"
        elif nbytes >= 400 * 1024:
            level = "ALERT"
        else:
            level = "NORMAL"
    else:
