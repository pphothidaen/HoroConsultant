// api/main.js — Vercel Node.js Middleend Gateway & CORS Proxy
...
module.exports = async (req, res) => {
  ...
  // 2. BaZi Interpret Route with Proxy to HF Spaces & Fallback
  if (req.method === "POST" && (url.includes("/bazi/interpret") || url.includes("/bazi"))) {
    try {
      ...
      // Graceful fallback if HF backend is waking up or unreachable
    }
  ...
  // 3. Generic passthrough response
  res.statusCode = 200;
  res.setHeader("Content-Type", "application/json; charset=utf-8");
  return res.end(JSON.stringify({
    status: "ok",
