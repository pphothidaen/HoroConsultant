...
#[pyfunction]
pub fn calculate_ming_shen_gong(lunar_month: i32, hour_branch_idx: usize) -> PyResult<(usize, usize)> {
    let m = (lunar_month - 1) as usize;
    let ming_idx = (2 + m + 12 - (hour_branch_idx % 12)) % 12;
    let shen_idx = (2 + m + (hour_branch_idx % 12)) % 12;
    Ok((ming_idx, shen_idx))
}

/// Calculate Zi Wei Star branch index based on Bureau Number and Lunar Day.
#[pyfunction]
pub fn calculate_zi_wei_star_branch(bureau_number: i32, lunar_day: i32) -> PyResult<usize> {
    if bureau_number <= 0 {
        return Ok(2);
    }
...
#[pyfunction]
pub fn calculate_14_main_stars(zi_wei_idx: usize) -> PyResult<Vec<(usize, Vec<String>)>> {
