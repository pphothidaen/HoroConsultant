...
4. **SDLC Management**: Spec breakdown into `plans/plan.md`, code review, and Task Board maintenance (`atomic_tasks.md`).
...
10. **Completed Branch Closeout**: After user-approved CI and review gates pass, merge the PR into `main` with a merge commit, delete its remote branch in the merge operation, fast-forward local `main`, and immediately delete the local source branch. Keep the branch when any gate or update fails; the read-only lifecycle guard proves `main` contains it before deletion.
11. **Atomic Tasks, Specialist & Skill Binding Mandate**: Decompose all broad tasks into atomic tickets in `atomic_tasks.md`, select matching specialists from the Agent Matrix, and explicitly bind required skills from the Skills Catalog before dispatching. Unbound dispatches fail closed.
