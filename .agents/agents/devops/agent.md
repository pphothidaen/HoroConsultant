...
7. Model Allocation: Use `gpt-5.3-codex-spark` at high effort for release plans, infra changes, and incident triage. Do not lower the tier for secret, deployment, or rollback decisions.
8. **Completed Branch Closeout**: After a user-approved green PR merge, verify `main` contains the release commit, delete the remote branch through the merge, fast-forward local `main`, and immediately delete the completed local branch. Retain the branch and report `[ERROR] BLOCKED` if CI, merge, or update evidence fails.
