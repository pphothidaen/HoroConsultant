...
Lesson 20 truth: original baseline 020 is DONE at `84b1dcf6125d13ed089ea2b6485fe059d6825d0a` and permanently `NON_TDD_RECONSTRUCTED`, never verified TDD. Mode-only `f1ed5ee` inherits that limitation. Safety v4 baseline 038 is DONE at immutable test-only commit `522beabd48b1c7395dedc09c3060a736041e9338`; its source 039 at `e14537311f349405f5c802a1e64017482b431d5c` is `REVIEW_BLOCKED` with 2 HIGH, 4 MEDIUM findings. v5 remediation baseline 046 is DONE at immutable test-only commit `69d852eb6dab654e4681f90556602efdedad34fd`; source 047 at `0949260c903eb9bc11ea75d577504b271736a2db` is `DONE_LOCAL` with focused 87/87 and full 1899 passed; independent review 048 is `READY_FOR_INTEGRATION` with all six findings verified closed.
...
Throughput policy: while a dependency-ready, non-overlapping lane exists, Root A maintains 3/3 native child-slot occupancy and immediately backfills every completed or blocked slot with the next ready microticket. AGY Orchestrator Concurrency Exception (Owner-approved 2026-08-30): for Read-Only / Research / QA Testing lanes, AGY orchestrator may scale concurrency up to 5–10 subagents simultaneously without risk of code conflicts; for Source Code Mutation lanes, concurrency remains strictly bounded to 2–3 subagents with non-overlapping file/directory ownership to guarantee Git history and worktree safety. Parallelism must not duplicate work, cross one-editor ownership, or bypass a test-only baseline, provenance allowlist, QA, review, main-only CI, deployment, or identity gate. Root B/AGY fan-out remains prohibited until its separate provider-proof microticket succeeds; static config and supervisor smoke are non-proof.
...
| RELEASE-VERIFIED-20260830-047-LESSON20-SAFETY-SOURCE-V5 | CRITICAL | L | developer | DONE_LOCAL | 046 VERIFIED | exactly `README.md`, `HOWTO.md`, `scripts/fail_fast_triage.py`, and `scripts/test_provenance_guard.py` from the 046 baseline; docs limited to POSIX-only CLI behavior and corrected `verify-pr --base/--head` syntax | remediate all six findings; focused 87/87 passed; full 1899 passed, 4 skipped; provenance guard PASSED | unverified 046, any unresolved finding, or stale/missing receipt | tests/other docs/integration/deploy |
| RELEASE-VERIFIED-20260830-048-LESSON20-SAFETY-REVIEW-V5 | CRITICAL | S | release_reviewer | READY_FOR_INTEGRATION | 047 | read-only review of exact 046/047 commits and bound receipts | independently verified closure of all six findings, scope, provenance, rollback, and no new HIGH/MEDIUM | finding, ambiguity, stale/unbound evidence, or source mutation | implementation/integration/deploy; never production approval |
...
GateImpactDecision v1 - Lesson 20 v5: schema=1; policy=2026-08-30.1; state=REVIEW_COMPLETE_READY_FOR_INTEGRATION; base=`e14537311f349405f5c802a1e64017482b431d5c`; head=`69d852eb6dab654e4681f90556602efdedad34fd`; diff_digest=`69d852eb6dab654e4681f90556602efdedad34fd`. Evidence: focused 87/87 passed; full 1899 passed, 4 skipped on source; candidate 1903 passed, 4 skipped; provenance guard verify-pr PASSED; secret scan 0 leaks; independent review READY_FOR_INTEGRATION with all six findings verified closed. `RUN` gates all green. Next: 040-INTEGRATE → 060-QA → 070-REVIEW → 080-MAIN.
...
Capacity policy: keep native child slots occupied whenever dependency-ready, non-overlapping microtickets exist (up to 5–10 for Read-Only/Research/QA Testing; 2–3 for Source Code Mutation with disjoint ownership under AGY Orchestrator Exception); immediately backfill a completed or blocked slot. Never create duplicate owners or bypass baseline/provenance dependencies to fill capacity. No AGY nested child may start before `GOV-BN-053-ROOTB-PROOF` records a fresh request, lease, quota observation, provider-bound receipt, and bounded no-write smoke; supervisor/static-config smoke is non-proof.
