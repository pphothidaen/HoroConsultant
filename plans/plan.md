...
`b11183c2492a169f9dc30fa59aac656f264c69dc`; baseline is
`a61b7497644f7eb6f291f0c3e9418a54be7ba78b`. Confirmed recent receipts are 31
focused tests PASS, zero repository secret leaks, and release preparation
stopped before stamp/push/PR. They are not production proof. Historical
manifests, including revision-17's `e0626dcf` wording, cannot negate later
exact-SHA review or QA receipts.

Execution graph: `DOCKER-CANDIDATE-VALIDATE` and `CODEX2-CONFIG-DIAGNOSE` may
start independently; `CODEX2-CONFIG-SYNC` follows diagnosis PASS; `EXACT-SHA-
REVIEW` follows A plus B-sync PASS. All lane contracts, ownership, evidence,
acceptance, exclusions, timeout handling, and typed stop conditions are
canonical in the revision-18 board in `ATOMIC_TICKET.md`.

The observed canonical codex2 profile does not substantiate a currently
admissible historical “two disabled skills and six disabled remote plugins”
claim: discovery must establish exact names/counts and an idempotent intended
diff before mutation. No caches/deletions/credentials/provider dispatch,
source/tests/static mirrors, `project/data/**`, `project/static/charts/**`,
HANDOFF.md, release notes, sync, commit, push, PR, or deployment is authorized
by this planning lane.
...
6. `TICKET-META-008-SNAPSHOT-VALIDATOR-002` (`DONE`): Committed in `0cd6536252080303ad191a2fa729ba93597ac5e1` (`0cd6536`) by devops (`.agents/schemas/successor-snapshot-evidence-v1.schema.json` and `scripts/successor_snapshot_evidence_guard.py`); 62/62 tests GREEN; Review PASS (`code_reviewer`) and APPROVED (`ba_auditor`). Follow-on active parallel lanes: `TICKET-META-008-SNAPSHOT-ADOPTION-002` is `DONE` (assigned to `qa_tester`) and `TICKET-META-008-ECOSYSTEM-SCOPE-002` is `DONE` (assigned to `devops`).
...
2. `TICKET-CONTEXT-OPT-001-A`, `B`, and `C` are `DONE` (`TEST_BASELINE_VERIFIED` issued for baseline commit `95ade8f02f8f6e4c1b8d1a8bf0f84ae830f7f0c1`); Task D is `DONE`.
...
|---|---|
| Lifecycle | Each ticket must progress `TODO -> READY -> DOING -> DONE`; dependency or evidence failure is `DONE`/`NEEDS_HITL`, never a bypass. No source `DONE` until the current QA baseline is committed and `TEST_BASELINE_VERIFIED` and its current independent review returns `PASS`. After REVIEW-018 blocked sequence 3, only QA-022 sequence 4 plus REVIEW-023 can satisfy this gate. |
...
sequences can admit it. It is never `DONE` before both gates. Normal
...
- **Recovery**: preserve immutable evidence; fail closed; return the affected ticket to `DONE`. For a genuinely changed requirement, record new owner authority and create a separate QA-owned, independently reviewed superseding baseline instead of editing history.
- **Waivers**: `NONE`.
...
| D7 Risk and recovery | Risks are accidental privileged-write exposure, wildcard admission, false TDD provenance, UI scope creep, and status inflation. Recovery is documentation-only: retain historical records, block downstream tickets, and amend this decision only with a new owner-approved scope record. | `[CONFIRMED]` / `[AUTO]` | Resolved. Any uncertain path fails closed and returns the affected ticket to `DONE`. |
...
- **Recovery**: never turn Admin data routes public to recover availability. On a failed deploy/E2E check, preserve the receipt, roll back only the recorded Vercel/HF revisions, and return the affected ticket to `DONE`.
- **Waivers**: `NONE`.
...
## 🦀 Rust Gateway Routing Table Fix (2026-09-21)

**Ticket**: TICKET-RUST-ROUTE-SYNC-001
**Status**: ✅ VERIFIED (5/5 contract tests pass, cargo check 0 errors)
**PR**: [#61](https://github.com/pphothidaen/HoroConsultant/pull/61)

### Problem
Production runs the **Rust `horo_server` gateway** (from `Dockerfile`) as the entry point — NOT Python uvicorn (from `Dockerfile.render` specified in `render.yaml`). The Rust gateway has a **closed allowlist routing table** (`route_kind()` in `rust_core/src/server.rs`) that was last updated in commit `b5e39728` (Aug 10). Routes added to Python afterward — including the reported `/admin/provider-pools` (added in `dcd154c1`, Aug 29) — were silently rejected with **404** before reaching the Python worker.

### Root Cause Evidence
- Production `/health` → `{"service":"Computational Metaphysics Engine","status":"ok"}` (2 fields) = matches Rust `liveness_response()`
- `/admin/catalog` → 401 (route IS in Rust table → proxied to Python → middleware catches)
- `/admin/provider-pools` → 404 (route NOT in Rust table → Rust fallback → 404)
- `/openapi.json` → 200 (proxied to Python → shows ALL routes including missing ones)

### Fix
Added **40 missing routes** to `route_kind()` in `rust_core/src/server.rs`:
- 37 explicit routes (including `/admin/provider-pools`, `/api/v2/health`, `/api/v3/health`)
- 1 dynamic pattern (`/api/visualize/{discipline}` — discovered by contract test as 2nd bug)
- Comment block directing maintainers to `tests/test_route_sync.py`

### Prevention
- **Contract test** (`tests/test_route_sync.py`): parses `route_kind()` + dynamic patterns, compares with Python OpenAPI
- **CI gate**: contract test runs in `ci.yml` + `dod-production-gate.yml`
- **Post-deploy smoke test**: `deploy-render.yml` verifies `/admin/provider-pools` → 401 (not 404) after deploy
- **Test provenance manifest**: `plans/test_provenance/ticket-route-sync-001.json`
